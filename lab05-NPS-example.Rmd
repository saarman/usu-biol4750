...
For now I'll proceed with the full geneind object.

### Genetic distance with adegenet::propShared:
```{r}
# Calculate genetic distance matrix
Dgen_matrix <- adegenet::propShared(Flr.genind)

# Convert to a distance vector (1 - proportion shared)
Dgen <- as.vector(1 - Dgen_matrix[lower.tri(Dgen_matrix)])
```
...
euclidean_distances <- dist(df_xy, method = "euclidean")
...
# Convert the matrix back into a data frame
df_xy <- as.data.frame(xy_matrix)
colnames(df_xy) <- c("X", "Y")  # Ensure column names are correct

# Calculate Euclidean geographic distances
Dgeo <- as.vector(lower.tri(dist(df_xy, method = "euclidean")))
```
